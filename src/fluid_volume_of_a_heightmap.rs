...
fn volume(heightmap: &Vec<Vec<i32>>) -> i32 {
    ...
    let cells = heightmap.iter().enumerate().flat_map(|(i, raw)| {
        raw.iter()
            .enumerate()
            .map(move |(j, heigh)| (*heigh, (i, j)))
    });
    let sorted_cells = cells
        .into_group_map()
        .into_iter()
        .sorted_by_key(|(k, vec)| *k);
    let i_max = heightmap.len();
    let j_max = heightmap[0].len();
    let mut increased_height:Vec<(usize, usize)> = vec![]; //increased_height_cells
    for (height, cells) in sorted_cells {
        let mut visited: HashSet<(usize, usize)> = HashSet::new();
        let mut floor_volume = 0;
        let cells = cells
            .into_iter()
            .chain(increased_height.drain(..))
            .collect_vec();
        for (i0, j0) in cells {
            ...
            if has_increase_height {
                ...
                increased_height.extend(increase_height_candidates.into_iter());
            }
        }
